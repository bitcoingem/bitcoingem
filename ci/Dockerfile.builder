...
# bitcoingem_hash
RUN git clone https://github.com/bitcoingempay/bitcoingem_hash
RUN cd bitcoingem_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} bitcoingem
RUN useradd -u ${USER_ID} -g bitcoingem -s /bin/bash -m -d /bitcoingem bitcoingem
...
RUN mkdir /bitcoingem-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /bitcoingem-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /bitcoingem-src
