...
export BUILDER_IMAGE_NAME="bitcoingem-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/bitcoingem"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_BGEM_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
