...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all BitcoinGem related functionality
...
/*
    ...
    allow endless transaction that would bloat BitcoinGem and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all BitcoinGem specific functionality
