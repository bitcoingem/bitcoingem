...
const char * const BITCOIN_CONF_FILENAME = "bitcoingem.conf";
const char * const BITCOIN_PID_FILENAME = "bitcoingemd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bitcoingem" is a composite category enabling all BitcoinGem-related debug output
                if(ptrCategory->count(std::string("bitcoingem"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinGem
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinGem
    // Mac: ~/Library/Application Support/BitcoinGem
    // Unix: ~/.bitcoingem
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinGem";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinGem";
#else
    // Unix
    return pathRet / ".bitcoingem";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitcoingem.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bitcoingem.conf
