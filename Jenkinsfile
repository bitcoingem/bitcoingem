...
// which are part of the BitcoinGem repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="bitcoingem-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /bitcoingem-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "bitcoingempay-bitcoingem/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from bitcoingempay-bitcoingem/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'bitcoingempay-bitcoingem/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from bitcoingempay-bitcoingem/develop"
            }
          }

          if (hasCache) {
            sh "cd /bitcoingem-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /bitcoingem-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /bitcoingem-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /bitcoingem-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /bitcoingem-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /bitcoingem-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /bitcoingem-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
